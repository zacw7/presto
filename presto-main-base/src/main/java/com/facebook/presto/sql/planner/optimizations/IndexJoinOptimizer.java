        ...
        implements PlanOptimizer
{
            ...
            extends SimplePlanVisitor<LookupVariableExtractor.Context>
    {
        ...
        // Traverse the non-equal join condition and extract the lookup variables. Return true if an unsupported condition is found.
        ...
        {
            // Node not expected to be part of Index pipeline.
            context.markIneligible();
            return null;
        }
        ...
        {
//            Optional<Map<ColumnHandle, Domain>> domains = node.getEnforcedConstraint().getDomains();
//            if (domains.isPresent() && !domains.get().isEmpty()) {
//                Map<VariableReferenceExpression, ColumnHandle> assignments = node.getAssignments();
//                Map<ColumnHandle, VariableReferenceExpression> flippedAssignments = assignments.entrySet().stream()
//                        .collect(toImmutableMap(Map.Entry::getValue, Map.Entry::getKey, (oldValue, newValue) -> {
//                            // Duplicate value in original map.
//                            context.markIneligible();
//                            return null;
//                        }));
//                for (ColumnHandle handle : domains.get().keySet()) {
//                    if (!flippedAssignments.containsKey(handle)) {
//                        context.markIneligible();
//                        return null;
//                    }
//                    context.getLookupVariables().add(flippedAssignments.get(handle));
//                }
//            }
            return null;
        }
